...
void GetBtlAxisValueHook(UE4::UObject* Context, UE4::FFrame& Stack, void* result) {
	...
	if (id > 0) {
		...
		if (std::string("BATTLE_MOVE_FORWARD_BACKWARD").compare(nameString) == 0) {
			*((float*)result) = mod->OldStates[id].Move.y;
		}
		else if(std::string("BATTLE_MOVE_LEFT_RIGHT").compare(nameString) == 0) {
...
{
	const std::string fnName = Frame->Node->GetName();
	if (obj && obj == ModActor && std::string("ModTick").compare(fnName) == 0)
	{
		...
		for (int i = 1; i < 4; i++) {
			...
			auto oldState = OldStates[i];
			auto justPressed = JustPressed[i];
			auto justReleased = JustReleased[i];
...
void MultiplayerMod::Tick(UE4::AActor *actor)
{
	Log::Info("TICK");

	if (ModActor != actor) return;
